...
string lidar_name = "lidar1";
string robot_name = "robot1";
string lidar_topic = "/velodyne_points1";
string lidar_scan_topic = "/registered_scan1";
string cmd_topic = "/cmd_vel1";
string odom_topic = "/state_estimation1";
string terrain_map_topic = "/terrain_map1";
string odomData_header_frame_id = "map1";
string odomData_child_frame_id = "sensor1";
string odomTran_frame_id = "map1";
string odomTran_child_frame_id = "sensor1";
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("camera_name", camera_name);
  nhPrivate.getParam("lidar_name", lidar_name);
  nhPrivate.getParam("robot_name", robot_name);
  nhPrivate.getParam("lidar_topic", lidar_topic);
  nhPrivate.getParam("lidar_scan_pub", lidar_scan_topic);
  ...
  nhPrivate.getParam("odomData_header_frame_id", odomData_header_frame_id);
  nhPrivate.getParam("odomData_child_frame_id", odomData_child_frame_id);
  nhPrivate.getParam("odomTran_frame_id", odomTran_frame_id);
  nhPrivate.getParam("odomTran_child_frame_id", odomTran_child_frame_id);

  ros::Subscriber subScan = nh.subscribe<sensor_msgs::PointCloud2>(lidar_topic, 2, scanHandler);

  ros::Subscriber subTerrainCloud = nh.subscribe<sensor_msgs::PointCloud2>(terrain_map_topic, 2, terrainCloudHandler);

  ros::Subscriber subSpeed = nh.subscribe<geometry_msgs::TwistStamped>(cmd_topic, 5, speedHandler);

  ros::Publisher pubVehicleOdom = nh.advertise<nav_msgs::Odometry>(odom_topic, 5);

  nav_msgs::Odometry odomData;
  odomData.header.frame_id = odomData_header_frame_id;
  odomData.child_frame_id = odomData_child_frame_id;

  tf::TransformBroadcaster tfBroadcaster;
  tf::StampedTransform odomTrans;
  odomTrans.frame_id_ = odomTran_frame_id;
  odomTrans.child_frame_id_ = odomTran_child_frame_id;
  ...
  cameraState.model_name = camera_name;
  gazebo_msgs::ModelState lidarState;
  lidarState.model_name = lidar_name;
  gazebo_msgs::ModelState robotState;
  robotState.model_name = robot_name;

  ros::Publisher pubScan = nh.advertise<sensor_msgs::PointCloud2>(lidar_scan_topic, 2);
